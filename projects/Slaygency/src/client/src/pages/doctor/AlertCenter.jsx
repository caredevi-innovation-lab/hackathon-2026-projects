...
export default function AlertCenter() {
  ...
  async function loadAlerts() {
    try {
      ...
      setError(err?.response?.data?.message || tx('Unable to fetch alerts from server.', 'सर्भरबाट अलर्ट विवरण ल्याउन सकिएन।'));
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleResolve = async (alertId) => {
    try {
      ...
      setError(err?.response?.data?.message || tx('Could not resolve this alert.', 'यो अलर्ट समाधान गर्न सकिएन।'));
    } finally {
      setRefreshingId('');
    }
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
      <div className="flex flex-col gap-6">
        <section className="bg-white border border-indigo-100 rounded-2xl p-5">
          <p className="text-xs tracking-wider text-indigo-500 font-semibold mb-2">{tx('Doctor Alerts', 'डाक्टर अलर्ट')}</p>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="m-0 text-3xl font-semibold text-slate-800">{tx('Alerts Center', 'अलर्ट केन्द्र')}</h1>
              <p className="m-0 mt-1 text-sm text-slate-500">
                {isNe
                  ? `${filteredActive.length} सक्रिय अलर्ट तुरुन्त ध्यान आवश्यक।`
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 flex items-center gap-2 w-full md:w-[320px]">
              ...
              <input
                ...
                placeholder={tx('Search alerts by patient or reason...', 'बिरामी वा कारण अनुसार अलर्ट खोज्नुहोस्...')}
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-xl bg-indigo-600 text-white p-4">
            <p className="m-0 text-xs tracking-wider text-indigo-100 font-semibold">{tx('Active', 'सक्रिय')}</p>
            <p className="m-0 mt-1 text-2xl font-semibold">{filteredActive.length}</p>
          </div>
          <div className="rounded-xl bg-red-50 border border-red-100 p-4">
            <p className="m-0 text-xs tracking-wider text-red-500 font-semibold">{tx('Critical', 'गम्भीर')}</p>
            <p className="m-0 mt-1 text-2xl font-semibold text-red-600">{criticalCount}</p>
          </div>
          <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-4">
            <p className="m-0 text-xs tracking-wider text-emerald-600 font-semibold">{tx('Resolved (Recent)', 'समाधान (हालका)')}</p>
        ...
        <section className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h2 className="m-0 text-lg font-semibold text-slate-800">{tx('Active Alerts', 'सक्रिय अलर्ट')}</h2>
          </div>

          {loading ? (
            <div className="p-5 text-sm text-slate-400">{tx('Loading alerts...', 'अलर्ट लोड हुँदैछ...')}</div>
          ) : filteredActive.length === 0 ? (
            <div className="p-8 text-center text-sm text-slate-500">{tx('No alerts match your current search.', 'हालको खोजसँग मिल्ने अलर्ट छैन।')}</div>
          ) : (
            <div className="divide-y divide-slate-100">
              {filteredActive.map((alert) => {
                const patientName = alert?.patient?.name || tx('Unknown Patient', 'अज्ञात बिरामी');
                  ...
                  <article key={alert._id} className="p-5 flex flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${isCritical ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'}`}>
                            {isCritical ? tx('High Risk', 'उच्च जोखिम') : tx('Alert', 'अलर्ट')}
                          </span>
                        </div>
                        <p className="m-0 text-sm text-slate-600">{alert.message}</p>
                        <p className="m-0 mt-1 text-xs text-slate-400">
                          {bpText ? `${tx('Latest BP', 'हालको BP')}: ${bpText} • ` : ''}
                          {tx('Created', 'सिर्जना')}: {formatTime(alert.createdAt)}
                        </p>
                        {reasons.length > 0 && (
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        ...
                        >
                          {refreshingId === alert._id ? tx('Resolving...', 'समाधान हुँदैछ...') : tx('Resolve', 'समाधान गर्नुहोस्')}
                        </button>
                      </div>
                    </div>
                  </article>
