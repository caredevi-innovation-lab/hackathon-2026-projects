...
export default function UsersPage() {
  ...
  const onToggleStatus = async (user) => {
    try {
      ...
      setToast({
        show: true,
        text: `User ${newActive ? 'activated' : 'deactivated'}.`,
        tone: 'warning',
      });
    } catch {
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex w-full flex-wrap items-center gap-3 rounded-2xl border border-[#cfddf1] bg-white p-3 shadow-[0_12px_24px_rgba(17,68,144,0.08)] lg:w-auto">
            <input
              ...
              className="w-full rounded-xl border border-[#c9d9ef] bg-white px-4 py-3 text-base text-[#214371] outline-none placeholder:text-[#7b8fae] focus:border-[#2d78d9] lg:min-w-[360px]"
            />
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="rounded-xl border border-[#c9d9ef] bg-white px-4 py-3 text-base text-[#214371] outline-none focus:border-[#2d78d9]"
            >
              <option>All</option>
              <option>Admin</option>
          ...
          <p className="m-0 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-[#4a6793] shadow-[0_6px_14px_rgba(17,68,144,0.08)]">
            ...
            <table className="w-full min-w-[720px] border-separate border-spacing-y-2 text-left">
              ...
              <tbody>
                {filtered.map((user) => (
                  <tr
                    key={user.id}
                    className="rounded-xl border border-[#e0e8f6] bg-white transition duration-200 hover:bg-[#f7faff]"
                  >
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg border border-[#cddcf1] bg-white px-3 py-2 text-xs font-semibold text-[#345886] hover:bg-[#f3f8ff]"
                        >
                          <IconEdit />
                          Edit
                        </button>
                        <button
                          ...
                          className="rounded-lg border border-[#cddcf1] bg-white px-3 py-2 text-xs font-semibold text-[#345886] hover:bg-[#f3f8ff]"
                        >
                          Toggle
                        </button>
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg border border-[#f0c8cf] bg-[#fff6f8] px-3 py-2 text-xs font-semibold text-[#b54057] hover:bg-[#ffeff3]"
                        >
                          <IconTrash />
                          Delete
                        </button>
