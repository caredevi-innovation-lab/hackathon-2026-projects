...
const vitalsRows = [
	{ metric: 'BP (mmHg)', w29: '118/76', w30: '120/80', w31: '124/82', w32: '155/95', critical: true },
			...
			<section className="min-w-0 p-3 sm:p-5">
				<header className="mb-4 grid gap-3 rounded-2xl border border-[rgba(168,166,206,0.24)] bg-[rgba(255,255,255,0.9)] p-3 shadow-[0_14px_35px_rgba(78,67,170,0.08)] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
					...
					<div className="flex items-center justify-end gap-3">
						...
						<button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[#f3f4fa] text-[#5e6782]">
							<IconBell />
						</button>
					</div>
				</header>
					...
					<header className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="m-0 mt-1 text-sm text-[#78819a]">Patient ID: #MC-2940 • 28 Years • 32 Weeks Gestation</p>
							</div>
						</div>
						<div className="flex flex-wrap gap-2">
							<button type="button" className="rounded-xl border border-[#d6d9ea] bg-white px-3 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-[#666f89]">
								Export Report
							</button>
							<button
								type="button"
					...
					<div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
						<section className="rounded-2xl border border-[rgba(168,166,206,0.22)] bg-white p-3">
							<div className="mb-3 flex items-center justify-between">
								...
								<button type="button" className="rounded-md border border-[#e5e7f2] bg-white px-2 py-1 text-xs text-[#7b839c]">
									▼
								</button>
							</div>
									...
									<article key={entry.title} className="rounded-xl border border-[#eceef7] p-3">
										...
										{entry.tags && (
											<div className="mt-2 flex gap-2">
												{entry.tags.map((tag) => (
													<span key={tag} className="rounded bg-[#f1f2f9] px-2 py-1 text-[0.63rem] font-semibold text-[#8188a0]">
														{tag}
													</span>
												))}
											</div>
										...
										{entry.quote && (
											<blockquote className="m-0 mt-2 border-l-2 border-[#6570ff] pl-2 text-xs italic text-[#6f7891]">{entry.quote}</blockquote>
										)}

										{entry.linkLabel && (
											<button type="button" className="mt-2 border-0 bg-transparent p-0 text-xs font-semibold text-[#5a62e8]">
												{entry.linkLabel}
											</button>
										)}
									</article>
							...
							<button type="button" className="mt-3 w-full rounded-xl border border-[#e6e8f4] bg-[#f9faff] py-2 text-xs font-semibold uppercase tracking-[0.08em] text-[#8b92aa]">
						...
						<aside className="grid gap-4">
							<section className="rounded-2xl border border-[rgba(168,166,206,0.22)] bg-white p-3">
										...
										<tbody>
											{vitalsRows.map((row) => (
												<tr key={row.metric} className="border-t border-[#eceef7]">
													...
													<td className={`px-2 py-2 ${row.critical ? 'font-semibold text-[#d22230]' : 'text-[#707895]'}`}>{row.w32}</td>
												</tr>
											))}
										</tbody>
									</table>
								...
								<button type="button" className="mt-2 w-full border-0 bg-transparent py-1 text-xs font-semibold text-[#5a62e8]">
									View Detailed Analytics
								</button>
							</section>

							<section className="rounded-2xl border border-[rgba(168,166,206,0.22)] bg-white p-3">
								<h4 className="m-0 text-sm font-semibold uppercase tracking-[0.08em] text-[#878ea6]">History Tools</h4>
								<input
									type="text"
							...
							<section className="rounded-2xl border border-[#5d55ff] bg-gradient-to-br from-[#615aff] to-[#4739ea] p-4 text-white shadow-[0_16px_30px_rgba(80,62,232,0.35)]">
								<p className="m-0 text-xs uppercase tracking-[0.1em] text-[#d6d4ff]">Active Risk Level</p>
