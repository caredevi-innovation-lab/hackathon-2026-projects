...
function GaugeChart({ score }) {
  // score 0-100, arc on a 220Â° range
  const RADIUS = 60;
  ...
  return (
    <div className="relative w-48 h-48">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-5xl font-semibold text-slate-800 leading-none">{score}</span>
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl w-full">
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <Link to="/patient-health-data-entry" className="bg-indigo-600 text-white font-semibold px-6 py-3 rounded-xl text-sm hover:bg-indigo-700 transition-colors inline-block">
                Log Health Data
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* Left â€” Gauge + Category */}
              <div className="lg:col-span-1 flex flex-col gap-6">
                {/* Score Card */}
                <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm text-center">
                  <h3 className="text-xs font-semibold text-slate-400  tracking-widest mb-4">Risk Score</h3>
                  <GaugeChart score={riskScore} />
                  <div className={`mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full font-semibold text-sm ${riskBadge}`}>
                    {riskCategory}
                  </div>
                  <p className="text-xs text-slate-400 mt-2">
                    AI Confidence: <span className="font-semibold text-slate-600">{assessment.confidence}%</span>
                  </p>
                </div>

                {/* Quick Vitals */}
                <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                  <h3 className="text-sm font-semibold text-slate-700 mb-4">Analyzed Vitals</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center py-2 border-b border-slate-50">
                      <span className="text-xs text-slate-500">Blood Pressure</span>
                      <span className="text-sm font-semibold text-slate-800">{assessment.bloodPressure}</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-slate-50">
                      <span className="text-xs text-slate-500">Hemoglobin</span>
                      <span className="text-sm font-semibold text-slate-800">{assessment.hemoglobin} g/dL</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-slate-50">
                      <span className="text-xs text-slate-500">Age</span>
                      <span className="text-sm font-semibold text-slate-800">{assessment.age} years</span>
                    </div>
              ...
              <div className="lg:col-span-2 flex flex-col gap-6">
                ...
                <div className={`rounded-2xl p-6 border ${riskBg}`}>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h2 className={`text-xl font-semibold mb-1 ${riskColor}`}>{riskCategory}</h2>
                ...
                {/* Recommendations */}
                <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                  <h3 className="font-semibold text-slate-800 mb-4">Personalized Recommendations</h3>
                  <ul className="space-y-3">
                    {RISK_TIPS[riskKey].map((tip, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <span className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-white text-xs font-semibold ${riskScore >= 60 ? 'bg-red-500' : riskScore >= 35 ? 'bg-amber-500' : 'bg-emerald-500'}`}>
                          {i + 1}
                        </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Link
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-xl text-sm text-center shadow-md shadow-indigo-200 transition-all"
                  >
                    Log New Health Data
                  </Link>
                </div>
