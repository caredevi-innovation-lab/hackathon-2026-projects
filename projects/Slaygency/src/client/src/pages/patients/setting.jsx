...
function StatusBadge({ message }) {
  ...
  const isOk = message.startsWith('✓');
  return (
    <div className={`px-4 py-3 rounded-xl text-sm font-medium border ${isOk ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
      {message}
    </div>
  );
}
...
export default function SettingsPage() {
  ...
  const handlePasswordChange = async (e) => {
    e.preventDefault();
    if (pwForm.newPassword.length < 8) { setPwMsg('New password must be at least 8 characters.'); return; }
      ...
      <div className="flex-1 flex flex-col overflow-y-auto relative w-full min-w-0">
        ...
        <header className="flex justify-between items-center py-4 px-4 md:px-8 bg-white border-b border-slate-100 sticky top-0 z-40 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Manage your profile and security</p>
          </div>
        ...
        <main className="p-4 md:p-8 max-w-4xl mx-auto w-full">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm mb-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
            <div className="relative shrink-0">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white text-3xl font-black shadow-md shadow-indigo-200">
                {initial}
              </div>
              <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full border-2 border-white" title="Active" />
            </div>
            <div className="flex-1 text-center sm:text-left">
              <h3 className="text-xl font-bold text-slate-800">{user?.name || 'Patient User'}</h3>
              <p className="text-sm text-slate-500 mt-0.5">{user?.email}</p>
              <div className="flex flex-wrap justify-center sm:justify-start gap-3 mt-3">
                <span className="bg-indigo-100 text-indigo-700 text-[11px] font-bold px-3 py-1 rounded-full">{user?.role || 'Patient'}</span>
                <span className="bg-emerald-100 text-emerald-700 text-[11px] font-bold px-3 py-1 rounded-full">Active Account</span>
                {user?.lastLoginAt && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <form onSubmit={handleProfileSave} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col gap-4">
              ...
              <StatusBadge message={profileMsg} />

              <button
                type="submit" disabled={profileSaving}
                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold py-3 rounded-xl text-sm shadow-md shadow-indigo-200 transition-all flex items-center justify-center gap-2 mt-auto"
              >
                {profileSaving ? (
                  <>
            ...
            <form onSubmit={handlePasswordChange} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col gap-4">
              ...
              {pwForm.newPassword && (
                <div>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-widest mb-1">Password Strength</p>
              ...
              <StatusBadge message={pwMsg} />

              <button
                type="submit" disabled={pwSaving}
                className="w-full bg-slate-800 hover:bg-slate-900 disabled:bg-slate-400 text-white font-bold py-3 rounded-xl text-sm transition-all flex items-center justify-center gap-2 mt-auto"
              >
                {pwSaving ? (
                  <>
                    <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
          ...
          <div className="mt-6 bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            <h3 className="text-sm font-bold text-slate-700 mb-4">Security Overview</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-700">Last Login</p>
          ...
          <div className="mt-6 bg-white rounded-2xl p-6 border border-red-100 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <button className="shrink-0 bg-red-600 hover:bg-red-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs transition-colors shadow-sm">
                Deactivate Account
              </button>
            </div>
          </div>
